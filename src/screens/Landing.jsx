...
function PastryTicker() {
  ...
  return (
    <div className="city-marquee pastry-ticker rounded-cut">
      <div className="city-marquee-track items-center gap-4 px-3 py-2">
        {loop.map((p, i) => (
          <img
            key={`${p.src}-${i}`}
            src={p.src}
            alt=""
            className="h-14 w-20 shrink-0 rounded-cut object-cover shadow-card"
          />
        ))}
      </div>
    </div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pt-1">
            {PLATES.slice(0, 4).map((p) => (
              <img
                ...
                src={p.src}
                alt=""
                className="h-12 w-[4.5rem] shrink-0 rounded-cut object-cover shadow-card"
              />
            ))}
          </div>
          ...
          {PLATES.map((p, i) => (
            <Reveal key={p.src} delay={i * 70} className="shrink-0">
              <figure className="gallery-card card-cut w-[220px] sm:w-[260px]">
                <div className="img-zoom-wrap ken-burns-wrap aspect-[4/3] overflow-hidden">
                  <img src={p.src} alt="" className="img-zoom ken-burns h-full w-full object-cover" />
                </div>
          ...
          {(Array.isArray(steps) ? steps : []).map((s, i) => (
            <Reveal key={s.n} delay={i * 80}>
              <div className="card-cut hover-lift hover-cut mask-reveal h-full overflow-hidden">
                <div className="img-zoom-wrap h-24 overflow-hidden">
                  <img
        ...
        <Reveal>
          <div className="card-cut overflow-hidden">
            <div className="baker-mosaic">
              {PLATES.map((p) => (
                <div key={p.src} className="img-zoom-wrap overflow-hidden">
                  <img src={p.src} alt="" className="img-zoom ken-burns h-full w-full object-cover" />
                </div>
              ))}
            </div>
