...
import { Button, EmptyState, FoodTile, Modal } from '../components/ui.jsx';
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {data.dishes.map((dish) => {
          ...
          return (
            <div key={dish.id} className="flex gap-3 rounded-3xl bg-white p-4 shadow-card">
              <FoodTile emoji={dish.emoji} accent={k.accent} className="h-14 w-14 shrink-0 text-2xl" />
              <div className="min-w-0 flex-1">
                <p className="font-extrabold">{dish.name}</p>
                <p className="text-sm text-mute">{dish.ingredients}</p>
                <p className="mt-1 text-sm font-bold">
                  {formatMoney(dish.price, currency, app.locale)} / {dish.unit}
                </p>
              ...
              <button
                ...
                disabled={out}
                onClick={() => add(dish)}
                className="h-11 shrink-0 self-center rounded-2xl bg-primary px-4 text-sm font-bold text-white disabled:bg-line disabled:text-mute"
              >
                +
              </button>
            </div>
